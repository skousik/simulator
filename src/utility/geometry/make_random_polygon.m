function P = make_random_polygon(Nvertices,offset,scale)
% P = make_random_polygon(Nvertices,offset,scale)
%
...
% with each polygon separated by a column of NaNs.
    ...
    if length(Nvertices) == 1
        ...
        for idx = 1:length(Nvertices)
            P = [P, nan(2,1), make_random_polygon(Nvertices(idx),offset(:,idx),scale(idx))] ;
        end
        P = P(:,2:end) ;
    end
end